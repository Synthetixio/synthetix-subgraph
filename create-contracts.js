...
program.action(async () => {
  ...
  let escrowContractsContent = `${readOnlyComment}export let contracts = new Map<string, string>();${newLine}`;
  ...
  for (let i = 0; i < NETWORKS.length; i++) {
    ...
    const addressResolverContracts = sortBy(
      getContractDeployments('AddressResolver', 0, Number.MAX_VALUE, NETWORKS[i]),
      'startBlock',
    );

    const addressResolverContract = addressResolverContracts[addressResolverContracts.length - 1];

    escrowContractsContent += `contracts.set(${newLine}${doubleSpace}'rewardEscrow-${
      NETWORKS[i]
    }',${newLine}${doubleSpace}'${rewardEscrowContract.address.toLowerCase()}'${newLine});${newLine}`;
    escrowContractsContent += `contracts.set(${newLine}${doubleSpace}'escrow-${
      NETWORKS[i]
    }',${newLine}${doubleSpace}'${synthetixEscrowContract.address.toLowerCase()}'${newLine});${newLine}`;
    escrowContractsContent += `contracts.set(${newLine}${doubleSpace}'addressresolver-${
      NETWORKS[i]
    }',${newLine}${doubleSpace}'${addressResolverContract.address.toLowerCase()}'${newLine});${newLine}`;
  }

  escrowContractsContent += readOnlyComment;
  const targetFile = path.join(__dirname, './generated', 'contracts.ts');
