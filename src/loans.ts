...
export function handleLoanPartiallyLiquidated(event: LoanPartiallyLiquidatedEvent): void {
  ...
  loanPartiallyLiquidatedEntity.liquidatedAmount = toDecimal(event.params.amountLiquidated);
  loanPartiallyLiquidatedEntity.liquidatedCollateral = toDecimal(event.params.collateralLiquidated);
...
export function handleCollateralDeposited(event: CollateralDepositedEvent): void {
  ...
  collateralDepositedEntity.collateralAmount = toDecimal(event.params.amountDeposited);
...
function addLoanEntity(event: LegacyLoanCreatedEvent, collateralMinted: string): Loan {
...
export function handleLoanCreatedEtherLegacy(event: LegacyLoanCreatedEvent): void {
  let loanEntity = addLoanEntity(event, 'sETH');
  loanEntity.collateralMinted = 'sETH';
  loanEntity.save();
}

export function handleLoanCreatedsUSDLegacy(event: LegacyLoanCreatedEvent): void {
  let loanEntity = addLoanEntity(event, 'sUSD');
  loanEntity.collateralMinted = 'sUSD';
  loanEntity.save();
}

function closeLoan(event: LegacyLoanClosedEvent, collateralMinted: string): void {
...
export function handleLoanClosedEtherLegacy(event: LegacyLoanClosedEvent): void {
...
export function handleLoanLiquidatedLegacy(event: LegacyLoanLiquidatedEvent): void {
  let loanLiquidatedEntity = new LoanLiquidated(event.transaction.hash.toHex() + '-' + event.logIndex.toString());
...
export function handleLoanPartiallyLiquidatedLegacy(event: LegacyLoanPartiallyLiquidatedEvent): void {
  let loanPartiallyLiquidatedEntity = new LoanPartiallyLiquidated(
    event.transaction.hash.toHex() + '-' + event.logIndex.toString(),
  );
...
export function handleCollateralDepositedLegacy(event: LegacyCollateralDepositedEvent): void {
  let collateralDepositedEntity = new CollateralDeposited(
    event.transaction.hash.toHex() + '-' + event.logIndex.toString(),
  );
...
export function handleCollateralWithdrawnLegacy(event: LegacyCollateralWithdrawnEvent): void {
  let collateralWithdrawnEntity = new CollateralWithdrawn(
    event.transaction.hash.toHex() + '-' + event.logIndex.toString(),
  );
  collateralWithdrawnEntity.amountWithdrawn = toDecimal(event.params.amountWithdrawn);
